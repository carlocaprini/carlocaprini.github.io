...
A couple of examples taken by my own experience? Estimating the number of people in a particular and well-defined environment and providing predictive maintenance have been for sure two of the most challenging and captivating use cases.

However, Machine Learning is not for all! There are situations in which it cannot help. Not because it is not the right solution to use, but just because the right conditions for making it work properly are missing. So "When should Machine Learning be used"? The answer is simple and should never be forgotten: **"Only when there is enough meaningful data available"**.
...
## When not to apply Machine Learning
