...
The root `404.html` is a Jekyll page using the global shell. It is explicitly `noindex, follow` and excluded from both sitemap outputs; generated validation treats noindex pages as intentionally outside the canonical sitemap. Local preview serves the built `_site` artifact directly, just like deployment, and returns that exact generated document with status `404` for unknown paths.
